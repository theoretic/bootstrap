 ...
 (function($) {
		...
		$.fn.jTPS = function ( opt ) {
				...
				$(this).data('tableSettings', $.extend({
						perPages:					   [25, 50, 100, 'все'],							  /*  the "show per page" selection */
						perPageText:			'На странице:',											   /*  text that appears before perPages links */
						perPageDelim:		   ' &nbsp; ',   /*  text or dom node that deliminates each perPage link  */
						...
						function () {
								...
								$(this).bind('click', 
										function () { 
												...
												$('>.nav .status', pT).html( '' + ( cPos + 1 ) + ' - ' + ( cPos + ePos ) + ' из ' + rowCount + '' );
												clearSelection();
												/*  callback function after pagination renderd */
												$(pT).data('tableSettings').clickCallback();
										}
				...
				$('>.nav .status', this).html( 'showing ' + ( cPos + 1 ) + ' - ' + ( cPos + ePos ) + ' из ' + rowCount );
